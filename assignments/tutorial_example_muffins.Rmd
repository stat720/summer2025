...
**The objective of this study is to find the best combination of oven temperature and banana quantity to bake muffins. The main priority, however, is to find differences between muffin recipe.** 
...
The treatment factors will be oven temperature (250, 400, and 500 F) and muffin recipe (control and extra banana). 
This treatment structure will define the same ANOVA table for the treatment sources of variability, regardless of the design (Table 1). 

```{r echo=FALSE}
t_trt <- data.frame(Source = c("Temperature", "Recipe", "Temp x Recipe", "Parallels", "Total"),
                    df = c("t-1 = 3-1 = 2", "r-1 = 2-1 = 1", "(t-1)(r-1) = 2", "N-t", "N-1"))

knitr::kable(t_trt, caption = "ANOVA table - Treatment Sources of Variability")
```
...
There are three different proposed approaches: a randomized complete block design, a split-plot design, and a strip-plot design. 
For demonstration, we will consider designs with 3 repetitions. 

### Randomized complete block design

A block design consists of group of experimental units that are approximately similar among them. 
...
I assume that day is the blocking factor and then, I will bake all treatments (i.e., all combinations of temperature and recipe) once each day.
...
$$y = \mu + T_i + R_j +TR_{ij} + b_k + \varepsilon_{ijk}, \\ b_j \sim N(0, \sigma^2_b), \\ \varepsilon_{ijk}  \sim N(0, \sigma^2_\varepsilon), $$
where $y_{ijk}$ is the observed height for 
...
Within a day, oven temperature treatment (i.e., 250, 400, 500) is repeated 3 times. 
Within each oven, muffin batters for recipe C and B are prepared independently for each oven batch. 
...
```{r echo=FALSE}
t_design <- data.frame(Source = c("Day (block)", "", "Error(oven)", "", "", "Error(oven x day)", "Total"),
                       df = c("d-1 = 3-1 = 2", " ", "(t-1)d = (3-1)*3 = 6", "", "", "(r-1)t*d = 9", "N-1 = 17"))
...
```{r echo=FALSE}
t_splitplot <- data.frame(Source = c("Day", "Temperature", "Error(oven)",
