...
- Sample size, design, etc. are important 
...
For this simulation, we assume that all $b_{j(k)}$, $u_k$ and $\varepsilon_{ijk}$ arise from a Normal distribution, with variances $\sigma_{b}^2 = 3$, $\sigma_{u}^2 = 35$, $\sigma_{\varepsilon}^2 = 10$.
...
3. Estimate the $\mu_i$s and test the type I error rate (count of times when $p<0.05$ when the truth was actually $\mu_i = \mu_{i'}$), and the statistical power (count of times when $p<0.05$ when the truth was $\mu_i \ne \mu_{i'}$). 
...
for (i in 1:n_sims) {
  ...
  df_me <- df_me %>%
    mutate(mu_t = case_when(trt == "t10" ~ 200, 
                            trt == "t11" ~ 200, 
                            trt == "t12" ~ 180, 
                            .default = 220)) %>% 
    left_join(df_envs) %>% 
