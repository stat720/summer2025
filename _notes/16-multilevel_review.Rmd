...
Just like banana bread, banana muffins are different to other types of muffins because their dough includes bananas, which sometimes makes it challenging to get a fluffy dough. 
If you don't manage your oven temperature and amount of bananas properly, you might end up with a dough that is too heavy and does not rise properly. 
We'll start with our go-to banana recipe and fine-tune the best baking temperature. 
...
The treatment mean for the $i$th temperature is $\mu_i = \mu + T_i$. That mean won't change under different design structures. What *might* change is the confidence interval around that mean.  

- First, recall the formula for a CI: $\theta \pm t_{df, \frac{\alpha}{2}} \cdot se(\hat{\theta})$
- For example, the CI for the differences between means at 400F - 300F $\mu_{400} - \mu_{300}$ is $(\mu_{400} -\mu_{300}) \pm 1.96 \cdot se(\widehat{\mu_{400} - \mu_{300}})$
