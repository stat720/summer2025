...
Yesterday we agreed on the following design: 
...
t_rcbd <- data.frame(Source = c("Day", "Temperature", "Banana", "TxB", "Error(oven)", "Total"),
                    df = c("r-1 = 3-1 = 2", "t-1 = 3-1 = 2", "b-1 = 2-1 = 1", "(t-1)*(b-1) = 2",
                           "(tb-1)*r - (t-1) - (b-1) - (t-1)*(b-1) = 15 - 2 -1 -2 = 7", "N-1 = 17"))
...
t_rcbd <- data.frame(Source = c("Day", "Temperature", "Error(oven)",
                    ...
                    df = c("r-1 = 3-1 = 2", "t-1 = 3-1 = 2", "6 -2 = 4",
                           "b-1 = 2-1 = 1", "(t-1)*(b-1) = 2", "(b-1)* t * r - (b-1) - (t-1)*(b-1) = 15 - 2 -1 -2 = 10", "N-1 = 17"))
...
```{r}
#get test t
qt(p = .975, df = 7) # df are df error(oven)
```

- The CI for the differences between means for normal and high banana $\mu_{\cdot 1} - \mu_{\cdot 2}$ is $(\mu_{\cdot 1} - \mu_{\cdot 2}) \pm 2.36 \cdot se(\widehat{\mu_{\cdot 1} - \mu_{\cdot 2}})$ 
